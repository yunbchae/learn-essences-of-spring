    ...
    public void run(String[] args) throws Exception {
        ...
        commandActions.put(Command.releasedYearBy, arguments -> {
            ...
            moviesReleasedYearBy.forEach(it -> {
                String data = String.format(messageSource.getMessage("application.commands.releasedYearBy.format", new Object[0], Locale.getDefault()), counter.getAndIncrement(), it.getTitle(), it.getReleaseYear(), it.getDirector(), it.getWatchedDate().format(Movie.DEFAULT_WATCHED_DATE_FORMATTER));
                output.println(data);
            });
